    ...
    F.prototype.config = {
        ...
        if (i.startTime = (new Date).getTime(), a.url) {
            ...
            a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)), i.loading(), t.ajax({
                ...
                data: d,
                dataType: "json",
                headers: {"token":layui.data(layui.config.tableName)['token']},
                success: function (t) {
                ...
                complete: function( xhr,data ){
                    layui.data(layui.config.tableName, {
                        key: "token",
                        value: xhr.getResponseHeader("token")==null?layui.data(layui.config.tableName)['token']:xhr.getResponseHeader("token")
                    })
                }
            })
